...
// Properties that ALL messages have
...
// Base metadata that ALL messages have
interface MetadataBase {
  message_ref: string;
  clip?: Clip;
}

// Specific Message Types (A Discriminated Union)
...
type CelebrationMessage = MessageBase & {
  type: 'celebration';
  metadata: MetadataBase & {
    // celebration always exists in this case
    celebration: {
      id: string;
      type: 'subscription_renewed' | (string & {}); // Allow other strings with suggestion
      total_months: number;
      created_at: string;
    };
  };
...
type ReplyMessage = MessageBase & {
  type: 'reply';
  metadata: MetadataBase & {
    // original_sender & original_message always exists in this case
    original_sender: {
      id: string;
      username: string;
    };
...
export interface ChatMessage {
  ...
  sender: {
    ...
    username: string;
    slug: string;
    identity: { color: string; badges: Badge[] };
  }
